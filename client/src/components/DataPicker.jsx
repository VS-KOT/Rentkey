...
const footer = () => {
  return (
    <div className="text-gray-500 font-semibold">
      <p className="md:inline leading-[3px] mt-3">
        <span className="text-pink">** </span>Dates which are disabled are already booked by{" "}
      </p>
...
const DatePickerRange = ({ setCheckIn, setCheckOut, disabledDates }) => {
  ...
  const disabledDate = (current) => {
    ...
    if (disabledDates && disabledDates.length > 0) {
      return disabledDates.some((date) => current.isSame(moment(date, "DD/MM/YYYY"), "day"));
    }
    return false;
  };
